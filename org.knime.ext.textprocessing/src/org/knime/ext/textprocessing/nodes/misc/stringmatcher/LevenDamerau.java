...
import org.knime.core.node.ExecutionContext;
    ...
    *
    * The LD-distance is a function to compare two words. There are
    * four possible edit operations. One letter can be deleted, one can
    * be inserted, one can be changed into another or two of them can be
    * swapped. To disable one of the operations you can set them to zero
    *
    * Each operation can be given an individual weight. But mind, because
    * of the optimal algorithm, it is necessary that the weight of 2
    * switchoperation
    * must be equal or larger than the sum of the weight of an insert
    * and a deletion.
    *
    * The method getNearestWord(word) gives a list of all words in
    * the biblist which have the smallest distance to the origin word
    * (this can be more than one)
    *
    * Using the method calculate the distance of the two given strings
    * is returned.
    *
    * @author Iris Adae
    *
...
//            NodeLogger.getLogger(StringMatcherNodeModel.class);
       ...
       /** 1 if the class works with an sortedlist
        *  2 if the class works with the datatable
        *  0 otherwise .*/
        ...
        * initializes all weights with 1
        *
        */
       public LevenDamerau() {
        m_wd = 1;
        ...
        * you can get a list of the nearest words in the list to
        * a give one by the method getnearest word.
        *
        * @param data the datatable which includes
        * the word list in which should be searched
        * @param col the column of the datatable
        * @param workincache 1 if the user wishes to work in cache,
        * it is much faster but can cause a java.lang.OutOfMemoryError
     * @param exec Executioncontext
     * @throws CanceledExecutionException if canceled by user
        */
       public LevenDamerau(final DataTable data,
                               final int col,
                               final boolean workincache,
        ...
        * @param ws the int weight for one switch
        */
       public void setweight(final int wd,
                 final int wi,
                 final  int wc,
                 final int ws) {
            m_wd = wd;
      ...
      */
        ...
        */
       public int calculate(final String string1,
        ...
        * @return the minimum of k+1 and the Levenshtein-Damerau distance
        *          of the two strings
        */
       public int calculate(final String string1,
                              final String string2,
        ...
        * @return the distance of the two strings
        */
       private int calculate(final char[] wordfromchar,
                             final char[] wordtochar) {
...
//         LOGGER.info(max++);
        ...
        int db, i1, j1, d;
        int inf = n * m_wd + m * m_wi + 1; // maximal distance (delete all
                                           // letters and insert all other)
        ...
        for (int i = 1; i <= n; i++) {
           ...
           for (int j = 1; j <= m; j++) {
              ...
              h[i + 1][j + 1] = min(h[i][j] + d, // if its cheapest to change
                                                   // the letter or do nothing
                        h[i + 1][j] + m_wi, // if its cheapest to insert the
                                            // letter
                        h[i][j + 1] + m_wd, // if its cheapest to delete the
                                            // letter
                        h[i1][j1] + (i - i1 - 1) // if its cheapest to delete
                                                 // all letters between(origin
                                                 // word), then swap, and than
                                                 // insert the other
                                                 // letters(searched word)
                               * m_wd + m_ws + (j - j1 - 1) * m_wi);
           }
            da[wordfromchar[(i - 1)] ] = i; // stores the position of the
                                                 // last letter
        }
       ...
       private int calculate(final char[] wordfromchar,
        ...
        /**
         * if wordfromchar is shorter then word tochar
         * the distance is at least the difference * m_wi
         */
          if (n < m && k < (m - n) * m_wi) { return k + 1; }

          /**
           * if wordfromchar is longer then word tochar
           * the distance is at least the difference * m_wd
           */
          if (m < n && k < (n - m) * m_wd) { return k + 1; }
        ...
        * @return an List of all words which have the smallest distance to z
        */
       private ArrayList<char[]> getNearestWord(final DataTable bibdata,
                                       final int bibcol,
                                     final char[] word) {
        ...
        for (DataRow row : bibdata) {
            if (!row.getCell(bibcol).isMissing()) {
                 char[] t = row.getCell(bibcol).toString().toCharArray();
       ...
       * There could be more than one word, which have the smallest distance
       *
       * @param t an by length sorted ArrayList
       *          (like the method sort produces)
       * @param z the string to be searched in the list t
       * @return an List of all words which have the smallest distance to z
       */
       private ArrayList<char[]> getNearestWord(
            final ArrayList<ArrayList<char[]>> t, final char[] z) {
           if (t.isEmpty()) {
               m_lastdist = Integer.MAX_VALUE;
        ...
        for (int i = 1; i <= 2 * m + 1; i++) {
            ...
            if (n >= 0 && n < t.size()) {
                ...
                /**
                 * if wordfromchar is shorter then word tochar the distance is
                 * at least the difference * m_wi if wordfromchar is longer then
                 * word tochar the distance is at least the difference * m_wd
                 */
                if (!(m < n + 1 && k < (m - 1 - n) * m_wi)
                        && !(m < n + 1 && k < (n + 1 - m) * m_wd)) {
     ...
     * @return an Arraylist with the following characteristics at the i.
     *         position of the list you find an list with all words of length
     *         i-1
     * @throws CanceledExecutionException
     * 
     */
    private ArrayList<ArrayList<char[]>> sort(final DataTable data,
            final int col) throws CanceledExecutionException {
        ...
        for (DataRow row : data) {
            ...
            if (!row.getCell(col).isMissing()) {
                String b = row.getCell(col).toString();
                int l = b.length() - 1;
                while (l >= maxlength) {
                    t.add(new ArrayList<char[]>(100));
                    maxlength++;
                }
                t.get(l).add(b.toCharArray());
            }
