 ...
 * @since 3.4
 */
public enum DocumentDataExtractor2 {
        ...
        AUTHOR_SET("Author set", new Extractor() {
            ...
            public DataCell getValue(final Document doc) {
                ...
                final List<DataCell> names = new ArrayList<DataCell>(authors.size());
                for (final Author author : authors) {
                    final String name = author.getFirstName() + " " + author.getLastName();
        ...
        CATEGORY_SET("Category set", new Extractor() {
            ...
            public DataCell getValue(final Document doc) {
                ...
                final List<DataCell> names = new ArrayList<DataCell>(categories.size());
                for (final DocumentCategory category : categories) {
        ...
        SOURCE_SET("Source set", new Extractor() {
            ...
            public DataCell getValue(final Document doc) {
                ...
                final List<DataCell> names = new ArrayList<DataCell>(sources.size());
                for (final DocumentSource source : sources) {
     ...
     *
     */
    private DocumentDataExtractor2(final String name, final Extractor extractor) {
    ...
    public static String[] getExtractorNames() {
        final DocumentDataExtractor2[] values = values();
     ...
     */
    public static DocumentDataExtractor2[] getExctractor(final String... names) {
        if (names == null) {
            return null;
        }
        final DocumentDataExtractor2[] extractors = new DocumentDataExtractor2[names.length];
        for (int i = 0, length = names.length; i < length; i++) {
            final String name = names[i];
            for (final DocumentDataExtractor2 extractor : values()) {
            ...
            if (extractors[i] == null) {
                throw new IllegalArgumentException("Invalid extractor name: " + name);
            }
        }
        return extractors;
    }
