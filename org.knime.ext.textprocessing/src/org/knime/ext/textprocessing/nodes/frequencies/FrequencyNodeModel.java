...
/**
 * This class represents the super class of all text frequency computation node models. Classes which extend
 * <code>FrequencyNodeModel</code> need to implement the method
 * {@link FrequencyNodeModel#initCellFactory(BufferedDataTable, ExecutionContext)} and take care of a proper
 * initialization of the used {@link org.knime.ext.textprocessing.nodes.frequencies.FrequencyCellFactory} instance. The
 * configure and execute procedure is done by the <code>FrequencyNodeModel</code>, classes extending this model do not
 * need to care about that. Once the used <code>FrequencyCellFactory</code> instance is initialized properly the rest is
 * done automatically.
 *
...
public abstract class FrequencyNodeModel extends NodeModel {
    ...
    /**
     * Creates a new instance of <code>FrequencyNodeModel</code> with given column name and flag if an integer column
     * has to be appended or a double column.
     */
    ...
    /**
     * Initializes the underlying <code>FrequencyCellFactory</code> and passes through the in data and an execution
     * context.
     *
     ...
     */
    protected abstract void initCellFactory(BufferedDataTable inData, ExecutionContext exec)
     ...
     */
    protected abstract DataTableSpec createDataTableSpec(final DataTableSpec inDataSpec);
    ...
    protected final BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec)
        ...
        if (m_cellFac == null) {
            throw new NullPointerException("CellFactory instance may not be null!");
        }
